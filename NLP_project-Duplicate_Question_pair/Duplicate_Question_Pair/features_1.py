...
def query_point_creator(q1,q2):
    ...
    q1_vectors = [word2vec_model.wv[token] for token in q1_tokens if token in word2vec_model.wv]
    if len(q1_vectors) == 0:
        q1_w2v = np.zeros(word2vec_model.vector_size)
    else:
    ...
    q2_vectors = [word2vec_model.wv[token] for token in q2_tokens if token in word2vec_model.wv]
    if len(q2_vectors) == 0:
        q2_w2v = np.zeros(word2vec_model.vector_size)
    else:
